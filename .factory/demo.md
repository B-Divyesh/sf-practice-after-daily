...
`demo:signal-garden:` and restores the sample run. **Start for real** discards
every demo key, leaves the demo URL, and uses the separate `signal-garden:`
local-storage namespace. Demo mode never reads or writes that real namespace.
